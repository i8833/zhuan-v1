...
const App = () => {
  ...
  // 修改handleCanvasMouseDown函数以优化竖排文字的点击检测
  const handleCanvasMouseDown = (event) => {
    ...
    texts.forEach((text, index) => {
      ...
      if (text.direction === 'vertical') {
        ...
        const textWidth = scaledFontSize * 1.5; // 增加宽度以便更容易点击
  ...
  // 优化触摸事件处理函数，使用requestAnimationFrame提高性能
  ...
  const handleTouchStart = (event) => {
    ...
    texts.forEach((text, index) => {
      ...
      if (text.direction === 'vertical') {
        ...
        const chars = text.content.split('');
        const textHeight = chars.length * scaledFontSize * 1.2;
        const textWidth = scaledFontSize * 1.5;
        
        if (
          touches[0].x >= text.x - textWidth/2 &&
          touches[0].x <= text.x + textWidth/2 &&
          touches[0].y >= text.y &&
        ...
        if (
          ...
          touches[0].y >= text.y - textHeight/2 &&
          touches[0].y <= text.y + textHeight/2
        ) {
          clickedTextIndex = index;
        }
